...
```bash
$ docker build -f docker/minitwit/Dockerfile -t Cosmotonic/minitwitserver .
```

  * Build the test client:
```bash
$ docker build -f docker/minitwit_client/Dockerfile -t Cosmotonic/minitwitclient .
```

Thereafter, via `docker compose`, the application, the client, Prometheus, and Grafana can be started. `docker compose up --build`
...
```bash
sed -i 's/cosmotonic/mydockerhubname/g' docker-compose.yml
```
